...
function allUniqueVersions(libBuilds, wwwBuilds, dojoBuilds, schedBuilds, pm2Procs, tmuxPanes, apacheHits) {
  const set = new Set([
    ...libBuilds.map(b => b.version),
    ...wwwBuilds.map(b => b.version),
    ...dojoBuilds.map(b => b.version),
    ...schedBuilds.map(b => b.version),
...
function printStatus(libBuilds, wwwBuilds, dojoBuilds, schedBuilds, pm2Procs, tmuxPanes, apacheHits = new Map()) {
  ...
  const schedByVer    = groupByVersion(schedBuilds);
  const pm2ByVer      = groupByVersion(pm2Procs);
  const tmuxByVer     = groupByVersion(tmuxPanes);
  const hasApache     = apacheHits.size > 0;

  const versions   = allUniqueVersions(libBuilds, wwwBuilds, dojoBuilds, schedBuilds, pm2Procs, tmuxPanes, apacheHits);
  ...
  let unusedExtraBytes = 0; // dojo + sched
  let unusedCount      = 0;
  ...
  for (const version of versions) {
    ...
    const scheds = schedByVer[version] || [];
    ...
    if (active && isLatest) {
      ...
      for (const d of dojos)  if (d.sizeBytes) unusedExtraBytes += d.sizeBytes;
      for (const s of scheds) if (s.sizeBytes) unusedExtraBytes += s.sizeBytes;
    }
    ...
    for (const d of dojos)  console.log(`  ${chalk.dim('dojo: ')} ${d.dirName}  ${chalk.dim(formatBytes(d.sizeBytes))}`);
    for (const s of scheds) console.log(`  ${chalk.dim('sched:')} ${s.dirName}  ${chalk.dim(formatBytes(s.sizeBytes))}`);
  ...
  const totalReclaimable = unusedLibBytes + unusedWwwBytes + unusedExtraBytes;
  ...
  if (totalReclaimable) {
    ...
    if (unusedExtraBytes) console.log(`  Reclaimable (dojo+sched): ${chalk.red.bold(formatBytes(unusedExtraBytes))}`);
...
function printCleanup(libBuilds, wwwBuilds, dojoBuilds, schedBuilds, pm2Procs, tmuxPanes, apacheHits = new Map()) {
  const pm2ByVer  = groupByVersion(pm2Procs);
  const tmuxByVer = groupByVersion(tmuxPanes);
  ...
  const unusedSched = schedBuilds.filter(b => isUnused(b.version));
  ...
  if (unusedLib.length === 0 && unusedWww.length === 0 && unusedDojo.length === 0 && unusedSched.length === 0 && apacheOnlyVersions.length === 0) {
  ...
  for (const version of versions) {
    ...
    const size   = (lib?.sizeBytes || 0) + (www?.sizeBytes || 0) +
                   [...dojos, ...scheds].reduce((s, d) => s + (d.sizeBytes || 0), 0);
    totalBytes += size;
    ...
    for (const s of scheds) console.log(`    ${chalk.dim('rm -rf')} ${s.path}`);
  }
...
function getUnusedBuilds(libBuilds, wwwBuilds, dojoBuilds, schedBuilds, pm2Procs, tmuxPanes, apacheHits = new Map()) {
  const pm2ByVer  = groupByVersion(pm2Procs);
  const tmuxByVer = groupByVersion(tmuxPanes);
  ...
  return {
    ...
    unusedSched: schedBuilds.filter(b => isUnused(b.version)),
  };
}
