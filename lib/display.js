...
function allUniqueVersions(libBuilds, wwwBuilds, pm2Procs, tmuxPanes, apacheHits) {
  const set = new Set([
...
function printStatus(libBuilds, wwwBuilds, pm2Procs, tmuxPanes, apacheHits = new Map()) {
  ...
  const versions   = allUniqueVersions(libBuilds, wwwBuilds, pm2Procs, tmuxPanes, apacheHits);
  ...
  if (unusedLibBytes || unusedWwwBytes) {
    console.log(`  Reclaimable (lib): ${chalk.red.bold(formatBytes(unusedLibBytes))}`);
    console.log(`  Reclaimable (www): ${chalk.red.bold(formatBytes(unusedWwwBytes))}`);
    console.log(`  Total reclaimable: ${chalk.red.bold(formatBytes(unusedLibBytes + unusedWwwBytes))}`);
  }
...
function printCleanup(libBuilds, wwwBuilds, pm2Procs, tmuxPanes, apacheHits = new Map()) {
  const pm2ByVer  = groupByVersion(pm2Procs);
  const tmuxByVer = groupByVersion(tmuxPanes);
  ...
  // Also collect versions that are "unused by processes" but have Apache hits
  ...
  if (unusedLib.length === 0 && unusedWww.length === 0 && apacheOnlyVersions.length === 0) {
    console.log(chalk.green('  No unused builds found.'));
    return;
  }

  // Show apache-active but process-inactive versions as a warning
...
function getUnusedBuilds(libBuilds, wwwBuilds, pm2Procs, tmuxPanes, apacheHits = new Map()) {
  const pm2ByVer  = groupByVersion(pm2Procs);
  const tmuxByVer = groupByVersion(tmuxPanes);
