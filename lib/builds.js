...
const SCHED_DIR_RE    = /^sched-(\d+\.\d+\.\d+[\w.-]*)$/;           // sched dirs: sched-2.5.5326
...
/**
 * Scan /var/www/html for sched-{BUILD_VERSION} dirs.
 * Ignores non-versioned dirs like sched-QA, sched-phoroszo, sched-rnixdorf.
 * Returns array sorted newest→oldest:
 * [{ version, dirName, path, sizeBytes }]
 */
function scanSchedBuilds(withSizes = true) {
  let entries;
  try {
    entries = fs.readdirSync(WWW_BASE);
  } catch (_) {
  ...
  return entries
    .filter(d => SCHED_DIR_RE.test(d))
    .map(d => {
      const m = d.match(SCHED_DIR_RE);
      const fullPath = path.join(WWW_BASE, d);
      return {
        version:   m[1],
        dirName:   d,
...
module.exports = { scanLibBuilds, scanWwwBuilds, scanDojoBuilds, scanSchedBuilds, versionCompare, formatBytes, getLatestBaseVersion };
